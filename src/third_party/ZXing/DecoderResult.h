...
*/
	...
	public:
		DecoderResult (DecodeStatus status)
			: _status (status) {}
		DecoderResult (ByteArray&& rawBytes, std::wstring&& text)
...
#define ZX_PROPERTY(TYPE, GETTER, SETTER)      \
	...
	DecoderResult&& SETTER (const TYPE& v)&& { \
		_##GETTER = v;                         \
		return std::move (*this);              \
	}                                          \
	DecoderResult&& SETTER (TYPE&& v)&& {      \
		_##GETTER = std::move (v);             \
		return std::move (*this);              \
	}
