 ...
 */
	...
	public:
		...
		BitMatrixCursor (const BitMatrix& image, POINT p, POINT d)
			: img (&image)
			, p (p) {
			setDirection (d);
		}
		...
		bool blackAt (POINT pos) const noexcept {
			return testAt (pos).isBlack ();
		}
		bool whiteAt (POINT pos) const noexcept {
			return testAt (pos).isWhite ();
		}

		bool isIn (POINT p) const noexcept {
			return img->isIn (p);
		}
		...
		Value edgeAtFront () const noexcept {
			return edgeAt (front ());
		}
		Value edgeAtBack () const noexcept {
			return edgeAt (back ());
		}
		Value edgeAtLeft () const noexcept {
			return edgeAt (left ());
		}
		Value edgeAtRight () const noexcept {
			return edgeAt (right ());
		}

		void setDirection (PointF dir) {
			d = bresenhamDirection (dir);
		}
		void setDirection (PointI dir) {
			d = dir;
		}
