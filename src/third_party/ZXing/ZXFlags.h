...
namespace ZXing {
	...
	class Flags {
		...
		constexpr inline Flags (Int other)
			: i (other) {}
		constexpr static inline unsigned numberOfBits (Int x) noexcept {
	...
	public:
		...
		iterator begin () const noexcept {
			return { i, BitHacks::NumberOfTrailingZeros (i) };
		}
		iterator end () const noexcept {
			return { i, BitHacks::HighestBitSet (i) + 1 };
		}

		constexpr inline bool operator== (Flags other) const noexcept {
			return i == other.i;
		}

		inline Flags& operator&= (Flags mask) noexcept {
			return i &= mask.i, *this;
		}
		inline Flags& operator&= (Enum mask) noexcept {
			return i &= Int (mask), *this;
		}
		inline Flags& operator|= (Flags other) noexcept {
			return i |= other.i, *this;
		}
		inline Flags& operator|= (Enum other) noexcept {
			return i |= Int (other), *this;
		}
		...
		constexpr static unsigned bitIndex (Enum flag) noexcept {
			return numberOfBits (Int (flag));
		}
		constexpr static Flags all () noexcept {
			return ~(unsigned(~0) << numberOfBits (Int (Enum::_max)));
		}
...
#define ZX_DECLARE_FLAGS(FLAGS, ENUM)                                                      \
	...
	constexpr inline FLAGS operator| (FLAGS::enum_type e1, FLAGS::enum_type e2) noexcept { \
		return FLAGS (e1) | e2;                                                            \
	}                                                                                      \
	constexpr inline FLAGS operator| (FLAGS::enum_type e, FLAGS f) noexcept {              \
		return f | e;                                                                      \
	}                                                                                      \
	constexpr inline bool operator== (FLAGS::enum_type e, FLAGS f) noexcept {              \
		return FLAGS (e) == f;                                                             \
	}                                                                                      \
	constexpr inline bool operator== (FLAGS f, FLAGS::enum_type e) noexcept {              \
		return FLAGS (e) == f;                                                             \
	}
