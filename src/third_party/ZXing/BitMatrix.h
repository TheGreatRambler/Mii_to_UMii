...
*/
	...
	public:
		...
		BitMatrix (int width, int height)
			...
			, _bits (((width + 31) / 32) * _height, 0) {}
#endif

		explicit BitMatrix (int dimension)
			: BitMatrix (dimension, dimension) {} // Construct a square matrix.
		...
		[[deprecated]] ByteMatrix toByteMatrix (int black = 0, int white = 255) const;
		...
		Row<data_t*> row (int y) {
			return { _bits.data () + y * _width, _bits.data () + (y + 1) * _width };
		}
		Row<const data_t*> row (int y) const {
			return { _bits.data () + y * _width, _bits.data () + (y + 1) * _width };
		}
#endif
	...
	*/
		...
		bool isIn (PointF p) const noexcept {
			return isIn (PointI (p));
		}
