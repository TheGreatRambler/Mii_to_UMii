...
namespace ZXing {
	...
	public:
		...
		int sum (int n = 0) const {
			return std::accumulate (_data, _data + (n == 0 ? _size : n), 0);
		}
		int size () const {
			return _size;
		}
		...
		int index () const {
			return static_cast<int> (_data - _base);
		}
		int pixelsInFront () const {
			return std::accumulate (_base, _data, 0);
		}
		int pixelsTillEnd () const {
			return std::accumulate (_base, _data + _size, 0) - 1;
		}
		bool isAtFirstBar () const {
			return _data == _base + 1;
		}
		bool isAtLastBar () const {
			return _data + _size == _end - 1;
		}
		bool isValid () const {
			return _data && _data + _size <= _end;
		}
		bool isValid (int n) const {
			return _data && _data + n <= _end;
		}
 ...
 */
	...
	struct BarAndSpace {
		...
		T& operator[] (int i) {
			return reinterpret_cast<T*> (this)[i & 1];
		}
		T operator[] (int i) const {
			return reinterpret_cast<const T*> (this)[i & 1];
		}
		bool isValid () const {
			return bar != T{} && space != T{};
		}
	};
