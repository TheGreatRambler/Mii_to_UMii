		...
		namespace RSS {
			...
			inline bool IsFinderPattern (const PatternView& v) {
				...
				auto isFinderPattern = [](int a, int b, int c, int d, int e, int g) {
					int w = 2 * (b + c), n = d + e;
					return w > 9 * n && w < 13 * n && std::max (b, c) < 10 * std::min (d, e) && a < 4 * d && 4 * a > n && g * 4 < 3 * n;
				};
				...
				return isFinderPattern (v[-1], v[0], v[1], v[2], v[3], guardLeft) || isFinderPattern (v[4], v[3], v[2], v[1], v[0], guardRight);
			}
