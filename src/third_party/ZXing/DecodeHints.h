 ...
 */
	...
	public:
		...
		DecodeHints ()
			...
			, _assumeCode39CheckDigit (0)
			, _assumeGS1 (0)
			, _returnCodabarStartEnd (0)
			, _binarizer (Binarizer::LocalAverage) {}

#define ZX_PROPERTY(TYPE, GETTER, SETTER)        \
	inline TYPE GETTER () const noexcept {       \
		return _##GETTER;                        \
	}                                            \
	inline DecodeHints& SETTER (TYPE v) {        \
		return _##GETTER = std::move (v), *this; \
	}
...
#undef ZX_PROPERTY
		...
		bool hasFormat (BarcodeFormat f) const noexcept {
			return _formats.testFlag (f);
		}
		bool hasNoFormat () const noexcept {
			return _formats.testFlag (BarcodeFormat::NONE);
		}
