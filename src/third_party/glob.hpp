...
namespace glob {
	...
	namespace {
		...
		// directory.
		static inline std::vector<fs::path> glob2 (const fs::path& dirname, const std::string& pattern,
			bool dironly) {
			// std::cout << "In glob2\n";
			std::vector<fs::path> result;
		...
		static inline std::vector<fs::path> glob (const std::string& pathname, bool recursive = false,
			...
			if (dirname.empty ()) {
				if (recursive && is_recursive (basename.string ())) {
					return glob2 (dirname, pathname, dironly);
				} else {
					return glob1 (dirname, pathname, dironly);
				}
			}
			...
			for (auto& d : dirs) {
				for (auto& name : glob_in_dir (d, pathname, dironly)) {
