...
namespace oead::util {
	...
	public:
		...
		BinaryReader (tcb::span<const u8> data, Endianness endian)
			: m_data{ data }
			, m_endian{ endian } {}
	...
	public:
		BinaryWriterBase (Endianness endian)
			: m_endian{ endian } {}
		...
		template <typename T, typename std::enable_if_t<!std::is_pointer_v<T> && std::is_trivially_copyable_v<T>>* = nullptr>
		void Write (T value) {
		...
		void AlignUp (size_t n) {
			Seek (util::AlignUp (Tell (), n));
		}
		void GrowBuffer () {
