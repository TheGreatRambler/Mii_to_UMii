...
namespace oead::util {
	...
	constexpr Result ExtractBits (const T src, const size_t begin, const size_t end) noexcept {
		return static_cast<Result> (((static_cast<Result> (src) << ((BitSize<T> () - 1) - end)) >> (BitSize<T> () - end + begin - 1)));
	}
	...
	public:
		...
		explicit BitCastPtrType (PtrType* ptr)
			: m_ptr (ptr) {}
	...
	class FlagBit {
	public:
		FlagBit (std::underlying_type_t<T>& bits, T bit)
			: m_bits (bits)
			, m_bit (bit) {}
	...
	public:
		...
		FlagBit<T> operator[] (T bit) {
			return FlagBit (m_hex, bit);
		}
