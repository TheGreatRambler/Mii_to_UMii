	...
	namespace byml {
		...
		public:
			Parser (tcb::span<const u8> data) {
				...
				m_string_table     = StringTableParser (m_reader, *m_reader.Read<u32> (offsetof (ResHeader, string_table_offset)));
		...
		struct WriteContext {
			WriteContext (const Byml& root, util::Endianness endianness)
				: writer{ endianness } {
			...
			struct StringTable {
				...
				u32 GetIndex (std::string_view string) const {
