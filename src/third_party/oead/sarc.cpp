...
namespace oead {
	...
	Sarc::Sarc (tcb::span<const u8> data)
		: m_reader{ data, util::Endianness::Big } {
	...
	Sarc::File Sarc::GetFile (u16 index) const {
		...
		file.data = m_reader.span ().subspan (m_data_offset + entry.data_begin, entry.data_end - entry.data_begin);
		return file;
	}
	...
	static const auto& GetAglEnvAlignmentRequirements () {
		static auto requirements = [] {
			...
			const auto tree = ryml::parse (yml::StrViewToRymlSubstr ({ info_tsv_file.begin (), info_tsv_file.size () }));
	...
	std::pair<u32, std::vector<u8>> SarcWriter::Write () {
		...
		size_t estimated_size = sizeof (sarc::ResHeader) + sizeof (sarc::ResFatHeader) + sizeof (sarc::ResFntHeader);
	...
	static bool IsSarc (tcb::span<const u8> data) {
		return data.size () >= 0x20 && (std::memcmp (data.data (), "SARC", 4) == 0 || (std::memcmp (data.data (), "Yaz0", 4) == 0 && std::memcmp (data.data () + 0x11, "SARC", 4) == 0));
	}
