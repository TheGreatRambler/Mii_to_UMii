...
namespace oead::yml {
	...
	static bool IsInfinity (std::string_view input) {
		return util::IsAnyOf (input, ".inf", ".Inf", ".INF") || util::IsAnyOf (input, "+.inf", "+.Inf", "+.INF");
	}
	...
	void LibyamlEmitter::Emit (yaml_event_t& event, bool ignore_errors) {
		if (!yaml_emitter_emit (&m_emitter, &event) && !ignore_errors) {
			throw std::runtime_error ("Emit failed: " + std::string (m_emitter.problem ? m_emitter.problem : "unknown"));
		}
	}
